...
\textit{\textbf{Example}: Pikachu is always an electric type. See lines 287 - 292 in the \nameref{S2_Appendix} for the corresponding JSON.}

% Image?

\textit{\textbf{Example}: Pikachu is never blue. Note that it is not inferred that Pikachus therefore are red. See lines 293 - 299 in the \nameref{S2_Appendix} for the corresponding JSON.}

% Image?

\textit{\textbf{Example}: Pikachu has a double-lobed tail in 50\% of the cases. See lines 300 - 313 in the \nameref{S2_Appendix} for the corresponding JSON.}
\subsection*{
Multilingualism
}
...
\textit{\textbf{Example}: Definition of a person and an organization. See lines 12 - 25 in the \nameref{S2_Appendix} for the corresponding JSON.}
\subsection*{
...
\textit{\textbf{Example}: Kangaskhan only occurs in Australia. See lines 120 - 169 in the \nameref{S2_Appendix} for the corresponding JSON.}

% Image?

\textit{\textbf{Example}: The rain form of Castform has a higher frequency in the rainy city of Bergen. See lines 325 - 368 in the \nameref{S2_Appendix} for the corresponding JSON.}
\subsection*{
Logical premises
}
...
\textit{\textbf{Example}: Pikachu caught during special events have hats, in contrast to those caught outside of such events. Other Pokémon (e.g. Honchkrow) always have hats. This allows for the characteristics of the hat to be used in identification, but only when one is not only left solely with Pokémon that can have hats, but when it has been established that the target Pokémon in fact is wearing one. See lines 257 - 271 in the \nameref{S2_Appendix} for the corresponding JSON.}
\subsection*{
Numerical values
}
...
\textit{\textbf{Example}: To allow for features such as the retrieval of the name of a Pokémon in different languages, one can use a Wikidata query. See lines 458 - 462 (service) and lines 52 - 55 (externalResource) in the \nameref{S2_Appendix} for the corresponding JSON.}
...
\textit{\textbf{Example}: The states in the tail character of Pikachu refer to images illustrating the different tail shapes. See lines 227 - 240 (states) and lines 465 - 511 (mediaElements) in the \nameref{S2_Appendix} for the corresponding JSON.}
\subsection*{
Descriptions
}
...
\textit{\textbf{Example}: A description can be used to explain to the user how to catch a Pokémon where relevant, by including a short description and a link to a page with more information. See lines 446 - 455 in the \nameref{S2_Appendix} for the corresponding JSON.}
\subsection*{
Followup
}
...
\textit{\textbf{Example}: Pikachu contains the colors yellow, black and red. The closely related Raichu contains yellow, brown, white and red. Setting the color character to be of non-exclusive type allows the user to indicate all the colors that each of them has. See lines 200 - 224 in the \nameref{S2_Appendix} for the corresponding JSON, and Fig.~\ref{fig4} for a graphical representation.}
