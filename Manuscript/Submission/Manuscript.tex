...
\textit{\textbf{Example}: Pikachu is never blue. See lines 271 - 277 in the \nameref{S2_Appendix} for the corresponding JSON.}
...
\textit{\textbf{Example}: The rain form of Castform has a higher frequency in the notoriously rainy Norwegian city of Bergen. See lines 302 - 347 in the \nameref{S2_Appendix} for the corresponding JSON.}
\subsection*{
Logical premises
}
...
\textit{\textbf{Example}: the weight of a Pokémon can be a useful characteristic as their weights vary between types. One can only weigh a Pokémon once it has been caught, however. The user needs to be aware of this so that in situations where a Pokémon cannot be caught (e.g. when identifying from a picture or being out of Pokéballs) such questions can be ignored. One can also add a guide on how to catch a Pokémon in order to weigh it. See lines 249 - 261 (character) and lines 420 - 432 (userRequirement) in the \nameref{S2_Appendix} for the corresponding JSON.}
